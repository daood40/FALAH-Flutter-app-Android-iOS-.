...
.PHONY: help dev worker lint typecheck security deps contract test audit ui failure leak isolation authz rbac invariants cors obs perms gate gate-all gate-list \
...
obs:  ## المراقبة: سجلٌّ بلا أسرارٍ ومقاييسُ بلا انفجارٍ عدديّ
	$(PY) obs_test.py
