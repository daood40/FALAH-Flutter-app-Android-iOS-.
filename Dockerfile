...
# الوثيقتان القانونيّتان — **لازمتان في الصورة لا في المستودع وحده.**
# يُصيّرهما `falah/legal.py` صفحتين على `/privacy` و`/terms`، والمتجران
# يشترطان رابطًا عامًّا يُفتح بلا حساب. وقد ردّت الصورةُ ٤٠٤ عليهما أوّلَ
# مرّةٍ لأنّ `.dockerignore` يستبعد `docs/` كلَّه — وأمسكه دخانُ الحاوية،
# لا قراءةُ شيفرة. ولا يُنسخ `docs/` كلُّه: الصورةُ تحمل ما تخدمه فقط.
COPY docs/PRIVACY_POLICY.md docs/TERMS_OF_SERVICE.md docs/
