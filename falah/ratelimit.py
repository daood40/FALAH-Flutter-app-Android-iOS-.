...
RATE = {
    ...
    # حدُّ الدخول القائم منذ P0 هو **لكل بريد** (٨ في ربع ساعة، في
    # `falah/auth.py`). وهو لا يمنع نمطًا آخر: من يجرّب ألفَ بريدٍ بكلمةٍ
    # واحدة (password spraying) لا يبلغ حدَّ أيّ بريدٍ منها أبدًا. فيُضاف
    # حدٌّ ثانٍ **لكل عنوان** إلى جانبه لا بدلًا منه — والاثنان معًا يسدّان
    # الوجهين: التخمينَ على حسابٍ واحد، والمسحَ على ألفِ حساب.
    "login":    _lim("LOGIN",    30, 900),     # لكل عنوان — في ربع ساعة
