...
**نطاق الفحص — ما يُفحص وبأي أداة:**

| المجال | الأداة | المخرَج |
|---|---|---|
| أسرارٌ في الشيفرة | هذا الملفّ (أنماطٌ على شكل السرّ لا اسمه) | critical/high |
| إعدادُ النشر (compose · كعكة · نطاق) | هذا الملفّ | high |
| صلاحياتُ ملفّات البيئة | هذا الملفّ | high |
| حارسُ CSRF على الكتابة | هذا الملفّ | critical |
| **ثغراتُ الاعتماديات** | `pip-audit` — `make deps` | يُسقط البوّابة |
| أنماطٌ خطِرة في الشيفرة | `ruff` (S102·S105·S106·S108·S301·S324·S501) | يُسقط البوّابة |
| تسريبُ الأخطاء والسجلّ | `leak_test.py` | يُسقط البوّابة |
| العزلُ والتزوير والمسارات | `audit.py` · `failure_test.py` | يُسقط البوّابة |

**ما لا يشمله أيٌّ من ذلك، ويُقال صراحةً:** لا مراجعةَ شيفرةٍ بشريةً
مستقلّة، ولا اختبارَ اختراقٍ، ولا تحليلَ تدفّقٍ (taint analysis)، ولا فحصَ
صورة الحاوية ضدّ قواعد الثغرات (trivy أو ما شابه).

**الاستثناءات وسببُ كلٍّ منها** — لا استثناءَ بلا سبب:

| المستثنى | لماذا |
|---|---|
| `tests.py` · `audit.py` · `ui_audit.py` · `failure_test.py` | تُصرّح بكلماتِ مرورٍ اختبارية وتبني مفاتيحَ وهمية عمدًا |
| `security_scan.py` | أنماطُ الكشف نفسها تطابق ما تكشفه |
| `launch/CREDENTIALS.md` | سياسةُ المفاتيح لا مفاتيح |
| `.env.example` | قالبٌ بلا قيم |
| قيمةٌ تبدأ بـ`http`/`/`/`$` | مرجعٌ لا سرّ — `TOKEN = "https://…/token"` |

والمجلّدات المتخطّاة (`raw` · `templates` · `exports` · `fonts` · `icons`)
مادّةٌ ثنائية أو خامٌ منزَّل، لا شيفرةَ فيها ولا إعداد.
"""
...
ALLOW = [
    ...
    (r"failure_test\.py$", "كسرٌ متعمَّد بحساباتٍ اختبارية"),
    (r"leak_test\.py$",    "يحقن سرًّا وهميًّا ليتحقّق أنه لا يخرج"),
]
...
def main():
    ...
    print("\n  نطاقُ هذا الفحص: أسرارٌ في الشيفرة · إعدادُ النشر · صلاحياتُ ملفّات\n"
          "  البيئة · حارسُ CSRF. ولا يشمل: ثغراتِ الاعتماديات (`make deps`)،\n"
          "  ولا الأنماطَ الخطِرة في الشيفرة (`make lint`)، ولا تسريبَ الأخطاء\n"
          "  (`make leak`) — ثلاثتُها بواباتٌ مستقلّة في `make gate`.\n"
          "  وما لا يُغطّيه شيءٌ منها: مراجعةٌ بشرية · اختبارُ اختراق ·\n"
          "  فحصُ صورة الحاوية. مذكورٌ لا مسكوتٌ عنه.")
